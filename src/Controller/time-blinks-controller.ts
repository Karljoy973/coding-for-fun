...
export class TimeBlinksController implements Controller {
  ...
  constructor(targettedView: defaultStrategyDigitalTimeView, emitter1: View) {
    this.targettedView = targettedView;
    this.emitter1 = emitter1;
    this._c = 0;

    this.emitter1.self?.addEventListener("click", this.emitter1ClickHandler);
  }
  ...
  emitter1ClickHandler = (e: MouseEvent) => {
    ...
    if (
      this._c == 0 &&
      this.targettedView.digitalViews[0].timestamp.className.includes(
        "isBlinking",
      )
    ) {
    }
    ...
    if (
      this._c == 0 &&
      this.targettedView.digitalViews[1].timestamp.className.includes(
        "isBlinking",
      )
    ) {
    ...
    if (
      ...
      this.targettedView.digitalViews[0].timestamp.className += " isBlinking ";
    }
